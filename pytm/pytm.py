...
with same properties, except name and notes""",
    ...
    def __init__(self, name, **kwargs):
        ...
        self.project_path = str(Path(os.path.join(path,".")).resolve()) # path current python file
        self.tmppath = self.project_path + "/../cache/tmp"
        self.toolpath = self.project_path + "/../tools/"
...
@enduml"""
    ...
    def process(self):
        ...
        if result.seq is True:
            this_tmppath = self.tmppath  + "_seq"
        ...
        if result.dfd is True:
            this_tmppath = self.tmppath  + "_dfd"
            with open(this_tmppath, "w", encoding="utf-8") as f:
                f.write( self.dfd(levels=(result.levels or set())) )
            os.chdir(self.toolpath + "/graphviz/")
            cmd_ = "dot.exe -Tpng -o {} {} ".format("../../cache/tmp_dfd.png", this_tmppath)
            print(cmd_)
            os.system(cmd_)
            os.chdir(self.project_path)
        ...
        if result.report is not None:
            this_tmppath = self.tmppath  + "_report.md"
            with open(this_tmppath, "w",encoding="utf-8") as f:
                f.write( self.report(result.report) )
